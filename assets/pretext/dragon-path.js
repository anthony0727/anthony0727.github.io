...
// Developer/anthony0727.github.io/assets/pretext/dragon-path-source.ts
var FONT = '16px Georgia, "Times New Roman", serif';
var LINE_HEIGHT = 24;
var PAD_X = 22;
var PAD_Y = 20;
var MIN_SLOT_WIDTH = 92;
var TEXT = [
...
var stage = document.getElementById("dragonStage");
if (!(stage instanceof HTMLDivElement))
  throw new Error("#dragonStage not found");
...
function render() {
  ...
  const height = Math.max(360, Math.floor(stage.clientHeight));
  const obstacles = createDragonObstacles(width, height);
  const lines = layoutAroundDragon(prepared, width, height, obstacles);
  svg.setAttribute("viewBox", `0 0 ${width} ${height}`);
  svg.replaceChildren(buildDragonSvg(width, height, obstacles));
  lineLayer.replaceChildren(...lines.map(createLineNode));
}
function createDragonObstacles(width, height) {
  const x0 = width * 0.18;
  const x1 = width * 0.82;
  const count = 8;
  const obstacles = [];
  for (let index = 0;index < count; index++) {
    ...
    const cx = x0 + (x1 - x0) * t;
    const cy = height * 0.5 + Math.sin(t * Math.PI * 1.85) * height * 0.16;
    const r = index === 0 || index === count - 1 ? 34 : 42 - Math.abs(0.5 - t) * 18;
    obstacles.push({ cx, cy, r });
  }
  obstacles.push({ cx: width * 0.82, cy: height * 0.28, r: 26 });
  obstacles.push({ cx: width * 0.865, cy: height * 0.24, r: 18 });
  return obstacles;
}
function layoutAroundDragon(preparedText, width, height, obstacles) {
  const lines = [];
  const base = { left: PAD_X, right: width - PAD_X };
  let cursor = { segmentIndex: 0, graphemeIndex: 0 };
  let y = PAD_Y;
  outer:
    while (y + LINE_HEIGHT <= height - PAD_Y) {
      const bandTop = y;
      const bandBottom = y + LINE_HEIGHT;
      const blocked = obstacles.map((obstacle) => circleIntervalForBand(obstacle, bandTop, bandBottom)).filter((slot) => slot !== null).sort((a, b) => a.left - b.left);
...
function circleIntervalForBand(obstacle, bandTop, bandBottom) {
  const top = bandTop - 5;
  const bottom = bandBottom + 5;
  if (top >= obstacle.cy + obstacle.r || bottom <= obstacle.cy - obstacle.r)
    return null;
  const minDy = obstacle.cy >= top && obstacle.cy <= bottom ? 0 : obstacle.cy < top ? top - obstacle.cy : obstacle.cy - bottom;
  if (minDy >= obstacle.r)
    return null;
  const dx = Math.sqrt(obstacle.r * obstacle.r - minDy * minDy);
  return {
    left: obstacle.cx - dx - 7,
    right: obstacle.cx + dx + 7
  };
}
...
function buildDragonSvg(width, height, obstacles) {
  const group = document.createElementNS("http://www.w3.org/2000/svg", "g");
  const bodyPath = document.createElementNS("http://www.w3.org/2000/svg", "path");
  bodyPath.setAttribute("d", buildBodyPath(obstacles));
  bodyPath.setAttribute("fill", "none");
  bodyPath.setAttribute("stroke", "#6e7f5b");
  bodyPath.setAttribute("stroke-width", "22");
  bodyPath.setAttribute("stroke-linecap", "round");
  bodyPath.setAttribute("stroke-linejoin", "round");
  bodyPath.setAttribute("opacity", "0.34");
  group.appendChild(bodyPath);
  const spine = document.createElementNS("http://www.w3.org/2000/svg", "path");
  spine.setAttribute("d", buildBodyPath(obstacles));
  spine.setAttribute("fill", "none");
  spine.setAttribute("stroke", "#314025");
  spine.setAttribute("stroke-width", "2");
  spine.setAttribute("stroke-linecap", "round");
  spine.setAttribute("stroke-linejoin", "round");
  spine.setAttribute("opacity", "0.72");
  group.appendChild(spine);
  const head = document.createElementNS("http://www.w3.org/2000/svg", "path");
  head.setAttribute("d", `M ${width * 0.84} ${height * 0.215} L ${width * 0.91} ${height * 0.245} L ${width * 0.845} ${height * 0.305} Z`);
  head.setAttribute("fill", "#6e7f5b");
  head.setAttribute("fill-opacity", "0.28");
  head.setAttribute("stroke", "#314025");
  head.setAttribute("stroke-width", "2");
  group.appendChild(head);
  const hornA = document.createElementNS("http://www.w3.org/2000/svg", "path");
  hornA.setAttribute("d", `M ${width * 0.852} ${height * 0.21} Q ${width * 0.835} ${height * 0.16} ${width * 0.8} ${height * 0.158}`);
  hornA.setAttribute("fill", "none");
  hornA.setAttribute("stroke", "#314025");
  hornA.setAttribute("stroke-width", "2");
  hornA.setAttribute("stroke-linecap", "round");
  group.appendChild(hornA);
  const hornB = document.createElementNS("http://www.w3.org/2000/svg", "path");
  hornB.setAttribute("d", `M ${width * 0.868} ${height * 0.215} Q ${width * 0.885} ${height * 0.164} ${width * 0.918} ${height * 0.162}`);
  hornB.setAttribute("fill", "none");
  hornB.setAttribute("stroke", "#314025");
  hornB.setAttribute("stroke-width", "2");
  hornB.setAttribute("stroke-linecap", "round");
  group.appendChild(hornB);
  const eye = document.createElementNS("http://www.w3.org/2000/svg", "circle");
  eye.setAttribute("cx", String(width * 0.872));
  eye.setAttribute("cy", String(height * 0.245));
  eye.setAttribute("r", "2.1");
  eye.setAttribute("fill", "#202018");
  group.appendChild(eye);
  const label = document.createElementNS("http://www.w3.org/2000/svg", "text");
  label.setAttribute("x", String(PAD_X));
  label.setAttribute("y", String(height - 12));
  label.setAttribute("fill", "#6f6658");
  label.setAttribute("font-size", "12");
  label.setAttribute("font-family", 'Georgia, "Times New Roman", serif');
  label.textContent = "dragon mode";
  group.appendChild(label);
  return group;
}
function buildBodyPath(obstacles) {
  if (obstacles.length === 0)
    return "";
  let path = `M ${obstacles[0].cx} ${obstacles[0].cy}`;
  for (let index = 1;index < obstacles.length - 2; index++) {
    const current = obstacles[index];
    const next = obstacles[index + 1];
    const controlX = (current.cx + next.cx) / 2;
    path += ` Q ${current.cx} ${current.cy} ${controlX} ${(current.cy + next.cy) / 2}`;
  }
  const neck = obstacles[obstacles.length - 3];
  const headBase = obstacles[obstacles.length - 2];
  path += ` Q ${neck.cx} ${neck.cy} ${headBase.cx} ${headBase.cy}`;
  return path;
}
