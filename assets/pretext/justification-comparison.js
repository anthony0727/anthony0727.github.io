// Developer/pretext/src/generated/bidi-data.ts
var latin1BidiTypes = [
  "BN",
  "BN",
  "BN",
...
// Developer/pretext/src/analysis.ts
...
// Developer/pretext/src/measurement.ts
...
// Developer/pretext/src/line-break.ts
...
// Developer/pretext/src/layout.ts
...
// Developer/pretext/pages/demos/justification-comparison.data.ts
var PARAGRAPHS = [
...
// Developer/pretext/pages/demos/justification-comparison.model.ts
...
var GRAPH_PARAGRAPH_INDEX = 2;
var GRAPH_WIDTH = 1040;
var GRAPH_HEIGHT = 280;
var GRAPH_PAD_X = 54;
var GRAPH_BASELINE_Y = 226;
...
function buildDemoFrame(resources, controls) {
  ...
  const graph = buildPathGraphFrame(resources.hyphenatedPreparedParagraphs[GRAPH_PARAGRAPH_INDEX], innerWidth, resources, GRAPH_PARAGRAPH_INDEX, PARAGRAPHS[GRAPH_PARAGRAPH_INDEX]);
  return {
    controls,
    css: {
...
function layoutParagraphOptimal(prepared, maxWidth, resources) {
  const result = solveOptimalParagraph(prepared, maxWidth, resources);
  return buildMeasuredLinesFromBreakPath(prepared, result.breakCandidates, result.breakIndices, maxWidth, resources.hyphenWidth);
}
function buildPathGraphFrame(prepared, maxWidth, resources, paragraphIndex, paragraphText) {
  const result = solveOptimalParagraph(prepared, maxWidth, resources);
  const segmentCount = Math.max(1, prepared.segments.length);
  const usableWidth = GRAPH_WIDTH - GRAPH_PAD_X * 2;
  const pathNodeSet = new Set([0, ...result.breakIndices]);
  const pathEdgeSet = new Set;
  let previousCandidate = 0;
  for (let index = 0;index < result.breakIndices.length; index++) {
    const candidate = result.breakIndices[index];
    pathEdgeSet.add(`${previousCandidate}:${candidate}`);
    previousCandidate = candidate;
  }
  const nodes = result.breakCandidates.map((candidate, candidateIndex) => {
    const progress = segmentCount === 0 ? 0 : candidate.segIndex / segmentCount;
    return {
      candidateIndex,
      segIndex: candidate.segIndex,
      kind: candidate.kind,
      x: GRAPH_PAD_X + progress * usableWidth,
      label: getCandidateLabel(candidate, candidateIndex),
      isPath: pathNodeSet.has(candidateIndex)
    };
  });
  const edges = result.edges.map((edge) => ({
    fromCandidateIndex: edge.fromCandidate,
    toCandidateIndex: edge.toCandidate,
    fromX: nodes[edge.fromCandidate].x,
    toX: nodes[edge.toCandidate].x,
    badness: edge.badness,
    isPath: pathEdgeSet.has(`${edge.fromCandidate}:${edge.toCandidate}`)
  }));
  return {
    width: GRAPH_WIDTH,
    height: GRAPH_HEIGHT,
    baselineY: GRAPH_BASELINE_Y,
    paragraphIndex,
    paragraphText,
    candidateCount: nodes.length,
    edgeCount: edges.length,
    pathLineCount: result.breakIndices.length,
    totalBadness: result.totalBadness,
    nodes,
    edges
  };
}
function solveOptimalParagraph(prepared, maxWidth, resources) {
  const segments = prepared.segments;
  const widths = prepared.widths;
  const segmentCount = segments.length;
  if (segmentCount === 0) {
    return {
      breakCandidates: [{ segIndex: 0, kind: "start" }, { segIndex: 0, kind: "end" }],
      edges: [{ fromCandidate: 0, toCandidate: 1, badness: 0 }],
      breakIndices: [1],
      totalBadness: 0
    };
  }
  ...
  const incomingEdges = Array.from({ length: candidateCount }, () => []);
  dp[0] = 0;
  for (let toCandidate = 1;toCandidate < candidateCount; toCandidate++) {
    ...
    for (let fromCandidate = toCandidate - 1;fromCandidate >= 0; fromCandidate--) {
      if (dp[fromCandidate] === Infinity)
        continue;
      const lineStats = getLineStatsFromBreakCandidates(segments, widths, breakCandidates, fromCandidate, toCandidate, resources.hyphenWidth, resources.normalSpaceWidth);
      if (lineStats.naturalWidth > maxWidth * 2)
        break;
      const edgeBadness = lineBadness(lineStats, maxWidth, resources.normalSpaceWidth, isLastLine);
      if (edgeBadness >= HUGE_BADNESS)
        continue;
      const edge = {
        fromCandidate,
        toCandidate,
      ...
      incomingEdges[toCandidate].push(edge);
      const totalBadness = dp[fromCandidate] + edgeBadness;
  ...
  const edges = [];
  for (let candidateIndex = 1;candidateIndex < incomingEdges.length; candidateIndex++) {
    const group = incomingEdges[candidateIndex];
    for (let edgeIndex = 0;edgeIndex < group.length; edgeIndex++) {
      edges.push(group[edgeIndex]);
    }
  }
  return {
    breakCandidates,
    edges,
    breakIndices,
    totalBadness: Number.isFinite(dp[candidateCount - 1]) ? dp[candidateCount - 1] : 0
  };
}
function buildMeasuredLinesFromBreakPath(prepared, breakCandidates, breakIndices, maxWidth, hyphenWidth) {
  const lines = [];
  let fromCandidate = 0;
...
function getCandidateLabel(candidate, candidateIndex) {
  if (candidate.kind === "start")
    return "start";
  if (candidate.kind === "end")
    return "end";
  if (candidate.kind === "soft-hyphen")
    return `h${candidateIndex}`;
...
// Developer/pretext/pages/demos/justification-comparison.ui.ts
function createDomCache() {
  ...
  return {
    ...
    pathGraph: getSvgElement("pathGraph"),
    metrics: {
    ...
    graphStats: createGraphStats(getHtmlElement("graphStats"))
  };
}
function renderFrame(dom, frame, normalSpaceWidth) {
  ...
  updateGraphStats(dom.graphStats, frame.graph);
  ...
  renderPathGraph(dom.pathGraph, frame.graph);
}
...
function getSvgElement(id) {
  ...
  if (!(element instanceof SVGSVGElement))
    throw new Error(`Missing SVGSVGElement #${id}`);
  return element;
}
...
function createGraphStats(container) {
  const candidatesValue = document.createElement("span");
  const edgesValue = document.createElement("span");
  const linesValue = document.createElement("span");
  const badnessValue = document.createElement("span");
  container.replaceChildren(createMetricRow("Candidates", candidatesValue), createMetricRow("Feasible edges", edgesValue), createMetricRow("Chosen lines", linesValue), createMetricRow("Path badness", badnessValue));
  return {
    candidatesValue,
    edgesValue,
    linesValue,
...
function updateGraphStats(panel, graph) {
  panel.candidatesValue.textContent = String(graph.candidateCount);
  panel.candidatesValue.className = "metric-value";
  panel.edgesValue.textContent = String(graph.edgeCount);
  panel.edgesValue.className = "metric-value";
  panel.linesValue.textContent = String(graph.pathLineCount);
  panel.linesValue.className = "metric-value good";
  panel.badnessValue.textContent = graph.totalBadness.toFixed(0);
  panel.badnessValue.className = "metric-value ok";
}
...
function renderPathGraph(svg, graph) {
  ...
  baseline.setAttribute("x1", "28");
  baseline.setAttribute("y1", String(graph.baselineY));
  baseline.setAttribute("x2", String(graph.width - 28));
  baseline.setAttribute("y2", String(graph.baselineY));
  baseline.setAttribute("stroke", "#d8cfbf");
  baseline.setAttribute("stroke-width", "1.5");
  root.appendChild(baseline);
  const caption = createSvgElement("text");
  caption.setAttribute("x", "28");
  caption.setAttribute("y", "28");
  caption.setAttribute("fill", "#6f6556");
  caption.setAttribute("font-size", "12");
  caption.setAttribute("font-family", '"Helvetica Neue", Helvetica, Arial, sans-serif');
  caption.textContent = `Paragraph ${graph.paragraphIndex + 1}: candidate breaks and chosen shortest path`;
  root.appendChild(caption);
  const excerpt = createSvgElement("text");
  excerpt.setAttribute("x", "28");
  excerpt.setAttribute("y", "48");
  excerpt.setAttribute("fill", "#a19380");
  excerpt.setAttribute("font-size", "11");
  excerpt.setAttribute("font-family", '"Helvetica Neue", Helvetica, Arial, sans-serif');
  excerpt.textContent = truncateText(graph.paragraphText, 118);
  root.appendChild(excerpt);
  for (let index = 0;index < graph.edges.length; index++) {
    root.appendChild(buildGraphEdge(graph, graph.edges[index]));
  }
  for (let index = 0;index < graph.nodes.length; index++) {
    root.appendChild(buildGraphNode(graph, graph.nodes[index]));
  }
}
function buildGraphEdge(graph, edge) {
  const path = createSvgElement("path");
  const span = Math.max(1, edge.toX - edge.fromX);
  const usableWidth = Math.max(1, graph.width - 108);
  const spanRatio = Math.min(1, span / usableWidth);
  const lift = 18 + Math.pow(spanRatio, 0.62) * 150;
  const controlX = (edge.fromX + edge.toX) / 2;
  const controlY = graph.baselineY - lift;
  path.setAttribute("d", `M ${edge.fromX} ${graph.baselineY} Q ${controlX} ${controlY} ${edge.toX} ${graph.baselineY}`);
  path.setAttribute("fill", "none");
  path.setAttribute("stroke", edge.isPath ? "#c65a28" : "#c8bcab");
  path.setAttribute("stroke-width", edge.isPath ? "3.4" : "1.15");
  path.setAttribute("stroke-opacity", edge.isPath ? "0.96" : edgeOpacity(edge.badness));
  path.setAttribute("stroke-linecap", "round");
  return path;
}
function buildGraphNode(graph, node) {
  const group = createSvgElement("g");
  const circle = createSvgElement("circle");
  circle.setAttribute("cx", String(node.x));
  circle.setAttribute("cy", String(graph.baselineY));
  circle.setAttribute("r", node.isPath ? "5.5" : "3.25");
  circle.setAttribute("fill", node.isPath ? "#1f1a16" : "#8c7f6c");
  circle.setAttribute("stroke", node.isPath ? "#f8f3eb" : "none");
  circle.setAttribute("stroke-width", node.isPath ? "1.6" : "0");
  group.appendChild(circle);
  if (!node.isPath)
    return group;
  const label = createSvgElement("text");
  label.setAttribute("x", String(node.x));
  label.setAttribute("y", String(graph.baselineY + 22));
  label.setAttribute("fill", "#5e5346");
...
// Developer/pretext/pages/demos/justification-comparison.ts
