...
// Developer/anthony0727.github.io/assets/pretext/home-dragon-source.ts
var FONT = '16px Georgia, "Times New Roman", serif';
var LINE_HEIGHT = 24;
var PAD_X = 22;
var PAD_Y = 20;
var MIN_SLOT_WIDTH = 94;
var HEAD_RADIUS = 18;
var BODY_RADIUS = 13;
var SEGMENT_COUNT = 18;
var SEGMENT_SPACING = 16;
var TEXT = [
...
var handle = document.getElementById("dragonHandle");
var shell = document.getElementById("dragonShell");
var stage = document.getElementById("dragonStage");
if (!(handle instanceof HTMLDivElement))
  throw new Error("#dragonHandle not found");
if (!(shell instanceof HTMLDivElement))
  throw new Error("#dragonShell not found");
...
function moveTargetFromClientPoint(clientX, clientY) {
...
function resetSegmentsNearTarget() {
...
function tick() {
  ...
  stepSegments();
  render();
  if (state.dragging || segmentsStillMoving())
    ensureAnimation();
}
...
function circleIntervalForBand(obstacle, bandTop, bandBottom) {
  ...
  if (top >= obstacle.y + obstacle.radius || bottom <= obstacle.y - obstacle.radius)
    return null;
  const minDy = obstacle.y >= top && obstacle.y <= bottom ? 0 : obstacle.y < top ? top - obstacle.y : obstacle.y - bottom;
  if (minDy >= obstacle.radius)
    return null;
...
function buildSvgFrame(width, height, segments) {
  ...
  snout.setAttribute("d", `M ${head.x + 8} ${head.y - 5} L ${head.x + 24} ${head.y} L ${head.x + 8} ${head.y + 5}`);
  ...
  hornA.setAttribute("d", `M ${head.x + 1} ${head.y - 7} Q ${head.x - 4} ${head.y - 18} ${head.x - 10} ${head.y - 18}`);
  ...
  hornB.setAttribute("d", `M ${head.x + 4} ${head.y - 6} Q ${head.x + 2} ${head.y - 19} ${head.x + 11} ${head.y - 21}`);
...
function buildBodyPath(segments) {
